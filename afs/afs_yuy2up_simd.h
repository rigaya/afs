...
static __forceinline __m128i get_even_uv_sse2(BYTE *ptr) {
	...
	x1 = _mm_loadu_si128((__m128i*)(ptr + 32));
	x0 = select_by_mask(x0, x1, _mm_srli_si128(xMask, 4));
	x0 = palignr_sse2(x0, x0, 2);
	x0 = _mm_shuffle_epi32(x0, _MM_SHUFFLE(1, 2, 3, 0));
	return x0;
}

static __forceinline __m128i get_even_uv_sse4_1(BYTE *ptr) {
	...
	x1 = _mm_loadu_si128((__m128i*)(ptr + 32));
	x0 = _mm_blend_epi16(x0, x1, MASK_INT_UV>>2);
	x0 = _mm_alignr_epi8(x0, x0, 2);
	x0 = _mm_shuffle_epi32(x0, _MM_SHUFFLE(1, 2, 3, 0));
	return x0;
}
#define get_even_uv_simd(ptr) ((simd & SSE41) ? get_even_uv_sse4_1(ptr) : get_even_uv_sse2(ptr))

static void __forceinline __stdcall afs_yuy2up_frame_simd(PIXEL_YC *dst, PIXEL_YC *src, int width, int pitch, int height, DWORD simd) {
	...
	for (int jh = 0; jh < height; jh++, src += pitch, dst += pitch) {
		...
		x1 = get_even_uv_simd(ptr_src +  0);
		x0 = _mm_shuffle_epi32(x1, _MM_SHUFFLE(0,0,0,0));
		BYTE *ptr_fin = (BYTE *)src + width * 6 - 48;
		for ( ; ptr_src <= ptr_fin; ptr_src += 48, ptr_dst += 48) {
			x2 = get_even_uv_simd(ptr_src + 48);
			x7 = x1;
			x6 = palignr_simd(x2, x1, 4);
			x7 = _mm_adds_epi16(x7, x6);
		
			x6 = _mm_adds_epi16(palignr_simd(x2, x1, 8), palignr_simd(x1, x0, 12));
		...
		if (remain_width < 8) {
			...
			ptr_src -= offset;
			ptr_dst -= offset;
			x0 = get_even_uv_simd(ptr_src - 48);
			x1 = get_even_uv_simd(ptr_src);
		}
