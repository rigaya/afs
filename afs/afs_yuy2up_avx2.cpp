...
static __forceinline __m256i get_even_uv_avx2_first(BYTE *ptr) {
	__m128i x0 = _mm_loadu_si128((__m128i*)(ptr +  2));
	__m256i y0 = _mm256_set_m128i(_mm_loadu_si128((__m128i*)(ptr + 66)), x0);
	__m256i y1 = _mm256_set_m128i(_mm_loadu_si128((__m128i*)(ptr + 50)), _mm_slli_si128(x0, 4));
	__m256i y2 = _mm256_loadu_si256((__m256i*)(ptr + 18));

	y0 = _mm256_blend_epi32(y0, y1, 0x80+0x10+0x02);
	y0 = _mm256_blend_epi32(y0, y2, 0x20+0x04);

	y0 = _mm256_shuffle_epi32(y0, _MM_SHUFFLE(2,3,0,1));
...
void __stdcall afs_yuy2up_frame_avx2(PIXEL_YC *dst, PIXEL_YC *src, int width, int pitch, int height) {	
	...
	__m256i y0, y1, y2, y3, y6, y7, yA0, yA1, yA2;
#if 0
	__m256i y4, y5;
#endif
	...
	for (int jh = 0; jh < height; jh++, src += pitch, dst += pitch) {
		...
		y0 = _mm256_broadcastd_epi32(_mm256_castsi256_si128(y1));
...
void __stdcall afs_yuy2up_frame_avx2(PIXEL_YC *dst, PIXEL_YC *src, int width, int pitch, int height) {	
	...
	const int MASK_UV_STORE_1 = 0x08 + 0x04;
	const int MASK_UV_STORE_2 = 0x80 + 0x40 + 0x02 + 0x01;
	__m256i y0, y1, y2, y3, y4, y5, y6, y7, yA0, yA1, yA2;
	...
	for (int jh = 0; jh < height; jh++, src += pitch, dst += pitch) {
		...
		BYTE *ptr_dst = (BYTE *)dst;
		BYTE *ptr_fin = (BYTE *)src + width * 6 - 96;
		y0 = get_even_uv_avx2_first(ptr_src);
		for ( ; ptr_src <= ptr_fin; ptr_src += 96, ptr_dst += 96) {
			y2 = get_even_uv_avx2(ptr_src + 96 - 6);
			y1 = _mm256_permute2x128_si256(y2, y0, (0x00<<4) + 0x03);
			y7 = _mm256_alignr_epi8(y1, y0, 4);
			y6 = _mm256_alignr_epi8(y1, y0, 8);
			y7 = _mm256_adds_epi16(y7, y6);
		
			y6 = _mm256_adds_epi16(_mm256_alignr_epi8(y1, y0, 12), y0);
			y6 = _mm256_subs_epi16(y6, y7);
			y6 = _mm256_srai_epi16(y6, 3);
